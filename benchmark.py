...
def do_benchmark(override_args):
    ...
    # If output file is not existed, create and add header 
    if not os.path.isfile(f"./{args.dataset}.csv"):
        with open(f"./{args.dataset}.csv", "w") as write_header:
            write_header.write("dims, emb_size, sampling_steps, recall_20, ckpt_name\n")
            
    with open(f"./{args.dataset}.csv", "a") as fout:
        dim_str = f'{args.dims}'.replace(',', '|')
        fout.write(
            "{},{},{},{},{}\n".format(
                dim_str,
                args.emb_size,
                ...
                '{}{}_lr{}_wd{}_bs{}_dims{}_emb{}_{}_steps{}_scale{}_min{}_max{}_sample{}_reweight{}_{}.pth' \
                    .format(args.save_path, args.dataset, args.lr, args.weight_decay, args.batch_size, dim_str, args.emb_size, args.mean_type, \
                    args.steps, args.noise_scale, args.noise_min, args.noise_max, args.sampling_steps, args.reweight, args.log_name)
            )
        )
