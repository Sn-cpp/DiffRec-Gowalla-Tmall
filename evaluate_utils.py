...
def print_results(loss, valid_result, test_result):
    ...
    # Adjustment 3: Replace print with tqdm.write

    if loss is not None:
        tqdm.write("[Train]: loss: {:.4f}".format(loss))
    if valid_result is not None: 
        tqdm.write("[Valid]: Precision: {} Recall: {} NDCG: {} MRR: {}".format(
                            '-'.join([str(x) for x in valid_result[0]]), 
                            '-'.join([str(x) for x in valid_result[1]]), 
                            '-'.join([str(x) for x in valid_result[2]]), 
                            '-'.join([str(x) for x in valid_result[3]])))
    if test_result is not None: 
        tqdm.write("[Test]: Precision: {} Recall: {} NDCG: {} MRR: {}".format(
